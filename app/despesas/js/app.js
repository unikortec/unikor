...
function toast(txt){ const b=$('#statusBox'); b.textContent = txt; }
...
$('#btnSalvarManual')?.addEventListener('click', async ()=>{
  ...
  // guarda categoria para autocompletar no futuro
  setCats([categoria, ...getCats()]);
  refreshCatDatalist();

  const total = itens.reduce((s,p)=>s+(p.valor||0),0);

  // 1) PDF sintético pro Drive
  try{
    toast('Gerando PDF…');
    // carrega jsPDF UMD sem SRI
    if (!window.jspdf) {
    ...
    }); // mantém JSON+TXT compatível
    // e também envia o PDF na mesma pasta/categoria:
    ...
    // 2) (Opcional) salvar no Firestore do tenant
    try{
      await saveManualToFirestore({ categoria, estabelecimento, itens, total });
...
// Forma de pagamento (pega do select)
function getFormaPagamento(){ return ($('#formaPagamento')?.value || 'OUTROS').toUpperCase(); }

// --- LINHAS PRODUTOS (igual ao seu, mantido) ---

// Salvar Manual (ADICIONA formaPagamento na gravação/Drive/Firestore)
$('#btnSalvarManual')?.addEventListener('click', async ()=>{
  const categoria = ($('#categoriaManual').value||'GERAL').trim();
  const estabelecimento = ($('#estabelecimento').value||'').trim();
  ...
  try{
    ...
    // Firestore (agora com formaPagamento e source)
    try{
  ...
  try{
    ...
    (res.itens.length ? res.itens : [{nome:'',valor:0}]).forEach(i=>{
      ...
      wrap.innerHTML = `
        ...
        <input class="produto-valor" type="number" step="0.01" placeholder="Valor (R$)" value="${(i.valor||0).toFixed(2)}"/>
        <button type="button" class="btn btn-add-linha">+</button>
        <button type="button" class="btn btn-rem-linha" title="Remover">–</button>`;
...
// NFC-e (QR) — igual ao seu; mantido.

// SW: FIX — enviar string 'SKIP_WAITING'
if ('serviceWorker' in navigator) {
  window.addEventListener('load', async ()=>{
    try{
      const reg = await navigator.serviceWorker.register('./sw.js', { scope: './' });
      ...
      reg.addEventListener('updatefound', ()=>{
        ...
        nw && nw.addEventListener('statechange', ()=>{
          if (nw.state==='installed' && navigator.serviceWorker.controller) {
            reg.waiting && reg.waiting.postMessage('SKIP_WAITING'); // <- FIX AQUI
          }
        });
      });
      navigator.serviceWorker.addEventListener('controllerchange', ()=>{
