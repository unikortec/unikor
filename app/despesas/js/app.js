...
// ===================== Config =====================
const VISUAL_FORMAT = 'pdf'; // 'pdf' (preferido) | 'jpeg'
...
// ===================== Auth (Firebase e-mail/senha + token Google p/ Drive) =====================
// O portal deve expor window.UNIKOR_AUTH.getCurrentUser() que resolve o usuário Firebase atual.
// Para o Drive, pedimos/renovamos o token Google sob demanda (reauth/link) mantendo o login principal por e-mail.
async function resolveUnikorUser(){
  if (window.UNIKOR_AUTH?.getCurrentUser) {
    const u = await window.UNIKOR_AUTH.getCurrentUser();
    if (u) return u;
  }
  if (window.auth?.currentUser) return window.auth.currentUser; // fallback
  return null;
}
...
async function boot(){
  ...
  CURRENT_USER = {
    ...
    role: u.role || u.claims?.role || 'user'
  };
...
function onSave(){
  ...
  let nota;
  if(editingId){
    const i = notas.findIndex(n=> n.id===editingId);
    if(i>=0){ notas[i] = { ...notas[i], ...base }; nota = notas[i]; }
  }else{
    nota = { id: String(Date.now()), origem: (dlg.dataset.modelo || 'manual'), itens: [], ...base };
    notas.push(nota);
  }
