export async function exportarPDF(rows){
  ...
  try{
    ...
    const R = 287;                // margem direita (A4 landscape ≈ 297mm - 10mm)
    ...
    const text  = (t,x,yy,opts)=>{ doc.setFont("helvetica","normal"); doc.setFontSize(10); doc.text(String(t), x, yy, opts); };
    const bold  = (t,x,yy)=>{ doc.setFont("helvetica","bold");   doc.setFontSize(10); doc.text(String(t), x, yy); };
    ...
    // ===== definição das colunas do bloco Cliente
    const cols = [
    ...
    const FW = cols.reduce((s,c)=> s + c.w, 0); // largura do conteúdo (não precisa ser W)

    const headerCliente = (yy)=>{
      fill(G_CLIENT); doc.rect(L, yy, W, 9, "F");             // fundo vai de margem a margem
      cols.forEach((c,i)=> headCell(c.label, X[i], c.w, yy)); // títulos nas larguras definidas
      hline(yy+9);
      return yy+12;
    };
    ...
    const IFW = icol.reduce((s,c)=> s + c.w, 0);

    const headerItens = (yy)=>{
      fill(G_ITEM); doc.rect(L, yy, W, 9, "F");               // fundo itens também de margem a margem
    ...
    rows.forEach(r=>{
      ...
      y = headerCliente(y); // cabeçalho cliente (cinza forte)
      ...
      y = headerItens(y);  // cabeçalho itens (cinza mais claro)
      ...
      itens.forEach(it=>{
        ...
        zebra = !zebra;
        if (zebra){ fill(G_ZEBRA); doc.rect(L, y-1, W, 7.5, "F"); }  // fundo vai de margem a margem
      ...
      // resumo do pedido (faixa cinza clara de margem a margem)
      ensure(12);
