...
function calcSubtotal({ desc, qtd, un, preco }){
  if (String(un||'').toUpperCase() === 'UN'){
    ...
    if (kgUn > 0){
      return (qtd * kgUn) * preco; // preco é R$/kg
    }
    // sem gramagem -> trata como un x preço unitário
    return qtd * preco;
  }
  // KG
  return qtd * preco;
...
function recalcTotal(){
  ...
  $("itemsBody").querySelectorAll("tr").forEach(tr=>{
    const sub = parseMoney(tr.querySelector(".it-sub").value);
    total += sub;
  });
...
export function addItemRow(item={}){
  ...
  const sub  = Number(item.subtotal|| (calcSubtotal({ desc, qtd, un, preco: pu })));

  const tr = document.createElement("tr");
  tr.innerHTML = `
    <td><input type="text" class="it-desc" value="${desc.replace(/"/g,"&quot;")}" /></td>
    <td><input type="number" min="0" step="0.001" class="it-qtd" value="${qtd}" /></td>
    <td>
      <select class="it-un">
    ...
    <td><input type="text" class="it-preco" value="${toMoney(pu)}" /></td>
  ...
  // listeners
  const inputsAuto = tr.querySelectorAll(".it-desc,.it-qtd,.it-un,.it-preco");
  inputsAuto.forEach(i => i.addEventListener("input", () => { tr.querySelector(".it-sub").dataset.dirty = ""; recalcRow(tr); recalcTotal(); }));
  tr.querySelector(".it-sub").addEventListener("input", (e)=>{ 
...
export async function salvarEdicao(atualizarLista){
  ...
  $("itemsBody").querySelectorAll("tr").forEach(tr=>{
    ...
    itens.push({
      descricao: desc, qtd, un, precoUnit: pu, subtotal: Number(sub.toFixed(2))
    });
  });

  const totalItens = itens.reduce((acc, it)=> acc + (it.subtotal||0), 0);
  const freteNum   = parseMoney($("mFrete").value);
  ...
  const payload = {
    ...
    // gravações auxiliares para o relatório/exports
