...
export async function montarPDF(){
  ...
  const SAFE_BOTTOM=280; // Reduzido para dar mais espaço
  let y=12;
  
  function ensureSpace(h){ 
    if (y+h>SAFE_BOTTOM){ 
      doc.addPage([72,297],"portrait"); 
      y=15; // Margem maior no topo da nova página
    } 
  }
  ...
  itens.forEach((it, idx)=>{
    ...
    ensureSpace(rowHi + (pesoTotalKg ? 8 : 0) + (it.obs ? 15 : 0)); // Melhor cálculo de espaço necessário
    ...
    subtotal += totalNum;
    if (idx < itens.length-1) y += 3; // Espaçamento entre itens
  });
